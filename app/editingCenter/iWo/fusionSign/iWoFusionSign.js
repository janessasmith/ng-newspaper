 ...
 * Rebuild:wang.jiang 2016-2-25
 */
"use strict";
angular.module('iWoFusionSignModule', ["trsngsweetalert", 'trsNavLocationModule'])
    .controller('iWoFusionSignCtrl', ["$stateParams", '$q', '$cacheFactory', '$scope', '$location', '$anchorScroll', 'trsHttpService', "trsconfirm", "$document", "$popover", "initVersionService", "$modal", "trsspliceString", "$timeout", "trsResponseHandle", 'initSingleSelecet', '$interval', 'editcenterRightsService', 'iWoService', 'editingCenterService', 'editNewspaperService', 'trsPrintService', 'storageListenerService', '$state', 'pagePreview', '$window', 'localStorageService', "editIsLock",
        function($stateParams, $q, $cacheFactory, $scope, $location, $anchorScroll, trsHttpService, trsconfirm, $document, $popover, initVersionService, $modal, trsspliceString, $timeout, trsResponseHandle, initSingleSelecet, $interval, editcenterRightsService, iWoService, editingCenterService, editNewspaperService, trsPrintService, storageListenerService, $state, pagePreview, $window, localStorageService, editIsLock) {
            initStatus();
            init();
            ...
            function initStatus() {
                ...
                $scope.params = {
                    ...
                    "methodname": "queryDocumentsOfMediaInShengQian",
            ...
            $scope.draftCorrelationView = function(item) {
                editNewspaperService.draftCorrelationViews(item.METADATAID, item.SITEID, 2);
            };
            /**
            ...
            /**
             * [cacheNewspaperDate description]缓存报纸数据已提高渲染速度
            ...
            /**
             * [batchTrail description]批量撤稿
             * @return {[type]} [description] null
             */
            $scope.revoke = function() {
                trsconfirm.inputModel("是否确认撤稿", "撤稿原因(可选)", function(content) {
                    var arrayMetaDataIds = trsspliceString.spliceString($scope.data.selectedArray, "METADATAID", ",");
                    var arrayChnlDocIds = trsspliceString.spliceString($scope.data.selectedArray, "CHNLDOCID", ",");
                    var ChannelId = $scope.data.selectedArray[0].CHANNELID;
                    draftManuscript(arrayChnlDocIds, arrayMetaDataIds, content, ChannelId);
                });
            };
            /**
            ...
            function requestData() {
                ...
                var params = {
                    serviceid: "mlf_fusion",
                    methodname: 'queryDocumentsInShengQian',
            ...
            $scope.singleSelect = function(item) {
                ...
                // if ($scope.data.selectedArray.indexOf(item) > -1) $scope.data.selectedArray.splice($scope.data.selectedArray.indexOf(item), 1);
                // else {
            ...
            function getTipInf() {
                ...
                if (loopedSite['isInterval'] === undefined || loopedSite['isInterval'] == true) {
                    var params = {
                        "serviceid": "mlf_fusion",
            ...
            //网站撤稿开始
            function draftManuscript(ChnlDocIds, metaDataIds, Opinion, ChannelId, item) {
                var params = {
                ...
                trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "get").then(function(data) {
                    trsconfirm.alertType("撤稿成功", "", "success", false);
                    requestData();
            ...
            //批量签发照排操作
            $scope.batchSignZhaopai = function() {
                singZhaopai($scope.data.selectedArray);
            };
            //单个签发照排操作
            $scope.singleZhaoPai = function(item) {
                singZhaopai([item]);
            };

            function singZhaopai(item) {
                editingCenterService.getSiteInfo($scope.status.currPanel.SITEID).then(function(data) {
                    if (data.ISZHAOPAI === '0') {
                        editNewspaperService.stopSignZp(item, function(result) {
                            ...
                            if (result.reports[0].RESULT) {
                                result.paperid = $scope.status.currPanel.SITEID;
                        ...
                        editNewspaperService.useSignZP(item, function(result) {
                            ...
                            if (result.reports[0].RESULT) {
                                result.paperid = $scope.status.currPanel.SITEID;
                                editNewspaperService.singZpInfo(result, $scope.status.currPanel.SITEID, function() {
                                    queryItemsBySiteId(false);
                                });
                            }
                        });
            ...
            //批量撤稿操作
            $scope.batchCancelDraft = function() {
                cancelDraft($scope.data.selectedArray);
            };
            //单个撤稿操作
            $scope.singleCancelDraft = function(item) {
                cancelDraft([item]);
            };
            //撤稿函数
            function cancelDraft(item) {
                var transferData = {
                    "title": "撤稿",
                    "opinionTit": "撤稿原因",
                    "items": item,
            ...
            function outSendingDraft(items) {
                ...
                var params = {
                    ...
                    methodname: "iFusionShenQianSendEmail",
                    Emails: userids,
            ...
            function getESSearchParams(isGetMore) {
                var esParams = {
                    ...
                    methodname: "queryForIwoFusionShenQianDoc",
                    searchParams: {
                        PAGESIZE: $scope.page.PAGESIZE + "",
            ...
            function getBtnRights(panel) {
                ...
                if (panel.MEDIATYPE == $scope.data.mediaType.website) {
                    ...
                    editcenterRightsService.initNewspaperListBtn("paper.sbg", panel.SITEID).then(function(data) {
                        $scope.status.btnRights = data;
                    });
                    editingCenterService.getSiteInfo(panel.SITEID).then(function(data) {
             ...
             * @param {[param]} [item] [description]   当前报纸item
             */
            ...
            /**
             * [isLockEdit description:报纸编辑前请求解锁]
            ...
            /**
             * [isWebistLockEdit description:网站编辑前请求解锁]
             * @param  {[type]} item [description:稿件对象]
             */
            $scope.isWebistLockEdit = function(item) {
                editIsLock.isLock(item).then(function(data) {
                    ...
                    var editParams = {
                        ...
                        siteid: item.SITEID,
                        status: $scope.status.webSitePlatformType
                    };
                    var editUrl = $state.href(editPath, editParams);
                    if (data.ISLOCK == "false") {
                        $window.open(editUrl);
