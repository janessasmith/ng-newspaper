 ...
 */
"use strict";
angular.module("productManageMentAppSiteRecycleBinModule", [])
    .controller("productManageMentAppSiteRecycleBinCtrl", ["$scope", "$state", "$stateParams", "trsHttpService", 'productMangageMentWebsiteService', 'trsconfirm', function($scope, $state, $stateParams, trsHttpService, productMangageMentWebsiteService, trsconfirm) {
        //返回上一页
        initStatus();
        initData();
                ...
                trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), reSiteParams, "post")
                    .then(function(data) {
                        trsconfirm.alertType("还原成功", "", "success", false, function() {
                            ...
                            $state.go("manageconfig.productmanage.app.site.recyclebin",$stateParams,{reload:true});
                        });
                    });
            });
        };
