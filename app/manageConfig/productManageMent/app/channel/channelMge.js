'use strict';
angular.module('productManageMentAppChannelModule', ['productManageMentAppChannelRouterModule','productManageMentAppChannelRecycleBinModule']).
controller('productManageMentAppChannelCtrl', ['$scope', '$rootScope', '$q', '$state', '$modal', '$stateParams', 'trsHttpService', 'trsconfirm', 'productMangageMentWebsiteService', 'localStorageService', "globleParamsSet", "websiteService","editingCenterService",function($scope, $rootScope, $q, $state, $modal, $stateParams, trsHttpService, trsconfirm, productMangageMentWebsiteService, localStorageService, globleParamsSet, websiteService,editingCenterService) {
    initStatus();
    initData();
    //获取栏目操作权限开始
        ...
        trsconfirm.inputModel("确认删除此频道", "输入删除的理由，可选填", function(data) {
            productMangageMentWebsiteService.inputPassword(function(password) {
                ...
                trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), deleteParams, "post")
                    .then(function(data) {
                        $rootScope.$broadcast("leftAddAppChannel", websiteItem);
    ...
    $scope.modifyViews = function(column, successFn, websiteChannel) {
        ...
        return modalInstance.result.then(function(result) {
            $rootScope.$broadcast("leftAddAppChannel", websiteChannel);
            $scope.params = result;
