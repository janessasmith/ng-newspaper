...
*/
'use strict';
angular.module("manageCfg.roleManageMent.permissionAssignment.modulePermission", ["manageCfg.roleManageMent.permissionAssignment.productAndModule", "manageCfg.roleManageMent.permissionAssignment.SavePerModule"])
    .controller("modulePermissionCtrl", ["$scope", "$timeout", "$state", "$modal", "$compile", "$element", "trsHttpService", "SweetAlert", "trsconfirm", "permissionAssignmentService", function($scope, $timeout, $state, $modal, $compile, $element, trsHttpService, SweetAlert, trsconfirm, permissionAssignmentService) {
        //声明将要保存权限的数组对象开始
        var selecetedChanls = {};
        //声明将要保存权限的数组对象结束
        initData();
        ...
        $scope.channelTreeOptions = {
            ...
            isLeaf: function(node) {
                return node.HASCHILDREN === "false" || !angular.isDefined(node.HASCHILDREN);
            },
            dirSelectable: true,
            allowDeselect: false
        };
