 ...
 *
 * Description 资源中心权限分配
 * Author:SMG 2016-6-03
 */
'use strict';
angular.module('manageCfg.roleManageMent.permissionAssignment.resourceCenter', ["manageCfg.roleManageMent.permissionAssignment.productAndModule", "manageCfg.roleManageMent.permissionAssignment.SavePerModule"])
    .controller("resourceCenterPermission", ["$scope", "$timeout", "$state", "$modal", "$compile", "$element", "trsHttpService", "SweetAlert", "trsconfirm", "permissionAssignmentService",
        function($scope, $timeout, $state, $modal, $compile, $element, trsHttpService, SweetAlert, trsconfirm, permissionAssignmentService) {
            //声明将要保存权限的数组对象开始
            var selecetedChanls = {};
            //声明将要保存权限的数组对象结束
            initData();
            ...
            function initData() {
                //获取左侧树开始
                getChannelTree($scope.roleData.ROLEID, function(data) {
            ...
            function getProductListData() {
                $scope.moduleName = "资源中心";
                $scope.classify = "source";
            }
            //获取从产品列表得到产品数据结束
            //取消选择树节点开始
