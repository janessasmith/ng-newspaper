...
*/
'use strict';
angular.module("manageCfg.roleManageMent.permissionAssignment.usermanage", ["manageCfg.roleManageMent.permissionAssignment.productAndModule", "manageCfg.roleManageMent.permissionAssignment.SavePerModule"])
    .controller("usermanagePerCtrl", ["$scope", "$timeout", "$state", "$modal", "$compile", "$element", "trsHttpService", "SweetAlert", "trsconfirm", "permissionAssignmentService", function($scope, $timeout, $state, $modal, $compile, $element, trsHttpService, SweetAlert, trsconfirm, permissionAssignmentService) {
        //声明将要保存权限的数组对象开始
        var selecetedChanls = {};
        //声明将要保存权限的数组对象结束
        initData();
        ...
        function getChannelTree(roleId, callback) {
            var params = {
                ...
                "methodname": "queryUserManageRight",
                "RoleId": $scope.roleData.ROLEID,
                "ModalName": "用户管理"
            };
